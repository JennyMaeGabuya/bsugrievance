...
$uip=$_SERVER['REMOTE_ADDR']; */
		$status = 1;
		$log = mysqli_query($bd, "insert into login_tbl(`admin_id`,`email`,`account_type`) values('" . $_SESSION['id'] . "','" . $_SESSION['login'] . "','$status')");
...
if (isset($_POST['change'])) {
    ...
    JOIN `admin_tbl` ON `adminpass`.`admin_id` = `admin_tbl`.`id` 
   SET `adminpass`.`password`='$password' WHERE `admin_tbl`.`email`='$email' and `admin_tbl`");
